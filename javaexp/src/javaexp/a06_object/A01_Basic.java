...
public class A01_Basic {
	...
	public static void main(String[] args) {
		 ...
		 * 						 - [메서드] : 웃다, 먹다, 말하다
		 * 3. 객체의 상호작용
		 * 		- 객체들은 서로간의 기능(동작)을 이용해서 데이터를 주고 받는 기능을처리한다.
		 * 		- 사람 객체 <==> 전자계산기 객체
		 * 		- 물건 객체 <==> 쇼핑몰 객체
		 * 
		 * # 객체와 클래스
		 * 1. 현실세계 비유
		 * 		현실세계 : 설계도 ==> 구체적인 사물 / 객체
		 * 			ex) 롯데월드 타워의 설계도를 동해서 롯데월드타워 건축
		 * 		자바 : 클래스 ==> 객체
		 * 			ex) class Tower{} ==> Tower t1 = new Tower();
		 * 2. 클래스에서 객체를 생성하기 위한 필드와 메소드가 정의 
		 * 3. 클래스로부터만들어진 객체를 해당 클래스의 인스턴스 (instance)
		 ...
		 * 		하나의 롯데월드타워 도면으로부터 서울, 부산, 제주도에 실제 건문을 건축할 수 있다.
		 * */
		
		// main() 에서만 실제 메모리를 만들고 처리가 된다.
		// 클래스를 기준으로 실제 객체를 p01이라는 인스턴스 / 참조변수로 객체를 만든다.
		/*Person p01 = new Person();
		System.out.println(p01); // heap영역의 특정한 주소
		p01.name = "홍길동";
		p01.age = 25;
...
// 이것을 통해서 만들어진 t1, t2, t3로 객체를 생성하고 loc에 서울/부산/제주로 선언하여
// heap 영역의 주소값과 각각의 loc를 출력해보세요
class Tower {
	String loc;
}